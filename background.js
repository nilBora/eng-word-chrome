...
function genericOnClick(info, tab) {
  ...
  ajaxSend(info['selectionText']);
}

function ajaxSend(word) {
	var xhr = new XMLHttpRequest();
	var params = "word="+word;
	xhr.open("POST", "http://eng.word.local.com/get/translate/", true);
	xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	xhr.onreadystatechange = function() {
	  if (xhr.readyState == 4)
	  {
		  var translateData = JSON.parse(xhr.responseText);
		  console.log(translateData['content']['translate']);
	  }
	}
	xhr.send(params);
}
